 ...
 */
public class ImageViewItemAnimator extends DefaultItemAnimator {
        ...
        if (changeFlags == FLAG_CHANGED) {
            for (Object payload : payloads) {
                if (payload instanceof String) {
                    return new InstructorItemAnimator.CharacterItemHolderInfo((String) payload);
                }
            }
        }
    ...
    public boolean animateChange(@NonNull RecyclerView.ViewHolder oldHolder,
        ...
        if (preInfo instanceof InstructorItemAnimator.CharacterItemHolderInfo) {
            InstructorItemAnimator.CharacterItemHolderInfo recipesItemHolderInfo = (InstructorItemAnimator.CharacterItemHolderInfo) preInfo;
            ImageAdapter.MyViewHolder holder = (ImageAdapter.MyViewHolder) newHolder;

            if (ImageAdapter.ACTION_LIKE_IMAGE_DOUBLE_CLICKED.equals(recipesItemHolderInfo.updateAction)) {
    ...
    private void animatePhotoLike(final ImageAdapter.MyViewHolder holder) {
        ...
        ObjectAnimator scaleLikeBackground = ObjectAnimator.ofPropertyValuesHolder(holder.root, PropertyValuesHolder.ofFloat("scaleX", 1.0f, 0.95f, 1.0f), PropertyValuesHolder.ofFloat("scaleY", 1.0f, 0.95f, 1.0f));
        scaleLikeBackground.setInterpolator(DECELERATE_INTERPOLATOR);
        scaleLikeBackground.setDuration(600);

        animatorSet.play(scaleLikeBackground);
